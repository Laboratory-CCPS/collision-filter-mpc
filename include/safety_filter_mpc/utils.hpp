#ifndef SAFETY_FILTER_UTILS_HPP
#define SAFETY_FILTER_UTILS_HPP
 ...
 */
template<typename T, size_t N>
inline std::array<T, N> create_alternating_array(const std::array<T, 2>& pattern_values) {
    std::array<T, N> alternated{};
    for (size_t i = 0; i < N; ++i) {
        alternated[i] = pattern_values[i % 2];
    }
    return alternated;
}
